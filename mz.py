...
def is_button_enabled(button):
    return "buttondiv_disabled" not in button.get_attribute("class").split() and button.is_enabled()
...
def login(driver):
    ...
    if not (username and password):
        LOG.error(
            "Username and/or password are not set. Did you create the `.env` file?"
        )
        driver.close()
        sys.exit(1)

    login_input = WebDriverWait(driver, TIMEOUT).until(
        EC.presence_of_element_located((By.ID, ID_USERNAME_INPUT))
    )
    login_input.clear()
...
def claim_tickets(driver):
    claim_button = WebDriverWait(driver, TIMEOUT).until(
        EC.presence_of_element_located((By.ID, ID_BUTTON_RECLAMAR))
    )
    time.sleep(10)
...
def automatically_play_in_event(driver, use_tactic=None, play_5_special=False):
    ...
    challenge_button = WebDriverWait(driver, WAIT_UNTIL_PREVIOUS_MATCH_ENDS).until(
    ...
    if is_button_enabled(challenge_button):
        ...
        play_button = WebDriverWait(driver, TIMEOUT).until(
            EC.presence_of_element_located((By.ID, ID_BUTTON_JUGAR))
        )
        if is_button_enabled(play_button):
            if use_tactic is not None:
                ...
                for option in tactics.find_elements_by_tag_name("option"):
                    ...
                    LOG.debug(
                        "Disculpa mostro, pero no sé qué táctica es %s. Vamos a salir con la default...",
                        use_tactic,
                    )
...
if __name__ == "__main__":
    ...
    with webdriver.Chrome(
        ...
        try:
            ...
            LOG.error(
                "Algo anduvo MUY mal, lo siento! No nos pudimos conectar! Escribile a magoya con tus reclamos y pasale este ST: %s.",
                err,
            )
        ...
        if got_tickets and AUTOMATIC_PLAY:
            ...
            if MATCHS_PER_CLAIM % 5 == 0:
                ...
                while games_to_play:
                    LOG.debug(
                        "Vamos al mejor de 5 partido' (de un total de %s)",
                        MATCHS_PER_CLAIM,
                    )
                    automatically_play_in_event(
                        driver, use_tactic=use_tactic, play_5_special=True
                    )
                    games_to_play -= 5
            else:
